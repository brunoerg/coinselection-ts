    ...
    for (let i = 0; i < MAX_TRIES; ++i) {
        ...
        if (backtrack) {
            while (curr_selection.length > 0 && !curr_selection[curr_selection.length - 1]) {
                curr_selection.pop();
                curr_available_value += utxos[utxos.length - 1].value;
            }
